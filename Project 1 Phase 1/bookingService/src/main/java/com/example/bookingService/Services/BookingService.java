...
public class BookingService {
            ...
            if (response.hasBody() && show != null) {
                ...
                if(remainSeats>0) {
                    try{
                        ...
                        String debitUrl = "http://host.docker.internal:8082/wallets/{userID}";
        ...
        try {
            ...
            //if user,show and booking exits then revert money to wallet and refill seats
                ...
                try{
                    ...
                    String debitUrl = "http://host.docker.internal:8082/wallets/{user_id}";
                    HttpHeaders headers = new HttpHeaders();
                    headers.setContentType(MediaType.APPLICATION_JSON);
   ...
   public ResponseEntity<?> deleteBookings() {
        ...
        for (Map.Entry<Integer, List<Booking>> entry : userbooking.entrySet()) {
            ...
            try {
                String debitUrl = "http://host.docker.internal:8082/wallets/{key}";// + "/" + userID + "/debit";
                HttpHeaders headers = new HttpHeaders();
                headers.setContentType(MediaType.APPLICATION_JSON);
                String requestBody = "{\"action\":\"" + "credit" + "\",\"amount\":" + final_price + "}";
                HttpEntity<String> requestEntity = new HttpEntity<>(requestBody, headers);
